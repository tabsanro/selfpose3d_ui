...
class ProjectLayer(nn.Module):
    def __init__(self, cfg):
        ...
        self.root_sample_grids = []
        self.root_bounding = None
    ...
    def get_voxel(self, heatmaps, meta, grid_size, grid_center, cube_size, flip_xcoords=None, rootnet=False):
        ...
        for i in range(batch_size):
            if not rootnet or not self.root_sample_grids:
            ...
            if rootnet:
                if not self.root_sample_grids:
                    for c in range(n):
                        self.root_sample_grids.append(sample_grids[c].clone())
                        self.root_bounding = bounding.clone()
                sample_grids = self.root_sample_grids
                bounding = self.root_bounding
    ...
    def forward(self, heatmaps, meta, grid_size, grid_center, cube_size, flip_xcoords=None, rootnet=False):
        cubes, grids = self.get_voxel(heatmaps, meta, grid_size, grid_center, cube_size, flip_xcoords=flip_xcoords, rootnet=rootnet)
        return cubes, grids